...
> 你的课题定位：`VLM-in-the-loop` 的 prompt 攻击优化，目标是突破 `text filter + image/output filter` 双防线，并做 `句子级/原子级` 拆解与重组。

| # | Paper | 和你课题的关系 | 可重点参考位置 |
| --- | --- | --- | --- |
| 1 | Perception-guided Jailbreak | 直接相关：可作为“句子级替换”策略起点 | `Perception gap mining`、替换规则、黑盒成功率对比 |
| 2 | Multimodal Pragmatic Jailbreak | 直接相关：文本+图像协同突破双防线 | 多模态威胁模型、双通道攻击流程、跨模型实验 |
| 3 | Unsafe Diffusion | 评测相关：可用于定义不安全目标集合 | 风险类别定义、数据构造、评测指标设计 |
| 4 | SurrogatePrompt | 直接相关：对“原子级片段替换”很有参考价值 | `Risky segment` 定位、代理短语生成、闭源绕过实验 |
| 5 | Adversarial Mislabeling | 间接相关：补充训练链路攻击面与防御假设 | 投毒流程、扰动标注机制、影响范围分析 |
| 6 | MMA-Diffusion | 直接相关：双防线绕过的强基线 | 文本攻击+图像攻击耦合方式、ASR 对比 |
| 7 | Black-Box Forgery on Semantic Watermarks | 间接相关：提示攻击后“后验取证”风险 | 黑盒伪造流程、检测器脆弱点分析 |
| 8 | CoprGuard | 间接相关：补充版权/检测侧约束条件 | 频域检测思路、鲁棒性与误报分析 |
| 9 | Mind the Trojan Horse | 直接相关：图像提示通道可作为第二突破路径 | IP-Adapter 攻击链、图像触发设计、黑盒验证 |
| 10 | OpenSDI | 直接相关：可作为你的“第二防线代理评测器” | 开放世界检测任务、检测/定位指标 |
| 11 | Six-CD | 间接相关：分析概念移除防御对攻击的影响 | 基准任务定义、失效案例、评测协议 |
| 12 | T2ISafety | 直接相关：可作为统一评测框架 | 毒性/公平/隐私维度指标与样本构造 |
| 13 | PLA | 直接相关：黑盒 prompt learning 可迁移到你的 VLM 回路 | 学习式提示优化目标、查询效率、成功率曲线 |
| 14 | JailbreakDiffBench | 直接相关：可用于标准化对比你方法 | 攻击类型划分、评测协议、榜单基线 |
| 15 | AutoPrompt | 直接相关：LLM 驱动自动红队与迭代优化 | 自动提示生成器、反馈回路、消融实验 |
| 16 | Feedback-Guided Prompt Iteration | 最相关：和你的“VLM 反馈优化”思路高度一致 | 反馈信号定义、迭代更新规则、收敛行为 |
| 17 | Prompting4Debugging | 直接相关：问题提示挖掘可做种子池 | 问题提示搜索策略、error-driven 调试流程 |
| 18 | Membership Inference on T2I | 间接相关：发布系统时的隐私风险补充 | 攻击假设、隐私指标、评测方法 |
| 19 | RPG-RT (Rule-based Preference Modeling) | 最相关：可参考“规则反馈->偏好优化”闭环 | 规则评分设计、偏好建模、迭代优化框架 |
| 20 | SneakyPrompt | 直接相关：token 级扰动可做原子级基线 | RL 扰动策略、黑盒反馈、攻击成本 |
| 21 | Nightshade | 间接相关：训练数据对抗与防守侧背景 | 定向投毒机制、概念偏移评估 |
| 22 | JailFuzzer | 最相关：可直接复用为“句子/原子级变异器” | 种子池、变异算子、oracle 反馈逻辑 |
| 23 | Unauthorized Data Usage Verification | 间接相关：系统合规与审计补充 | 黑盒验证协议、统计判别方法 |
| 24 | Glaze | 间接相关：防守方输入保护会影响攻击前提 | 扰动机制、鲁棒对抗场景 |
| 25 | Prompt Stealing | 相关：可用于评估你方法的提示泄露风险 | 提示重建管线、泄露度量指标 |
| 26 | Cross-Modal Prompt Inversion | 直接相关：文本/图像统一威胁模型可扩展你的框架 | 跨模态反演流程、跨任务评测 |
| 27 | Exposing the Guardrails (DALL·E) | 最相关：双防线结构与绕过路径的参考模板 | 过滤链路逆向、侧信道分析、攻击构造 |
| 28 | USD (Scene-Graph NSFW Detection) | 直接相关：可实现你的“输出侧检测代理器” | 场景图检测流程、关系级判定逻辑 |
| 29 | Bridging the Gap in VLM Unsafe Concept ID | 最相关：直接指导“VLM 作为前置审核器”的能力边界 | 跨模态识别差距、PPO 对齐方案 |
| 30 | Identifying Provenance of Generative T2I Models | 间接相关：用于结果归因与发布后追责 | 黑盒谱系判定、统计检验流程 |

### 你现在最该优先复用的 8 篇（按直接性排序）

1. `Feedback-Guided Prompt Iteration with VLM`
2. `RPG-RT`
3. `JailFuzzer`
4. `PLA`
5. `SurrogatePrompt`
6. `Perception-guided Jailbreak`
7. `MMA-Diffusion`
8. `Exposing the Guardrails`
